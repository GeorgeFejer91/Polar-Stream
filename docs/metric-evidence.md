...
The breathing processor follows the original Polar app's ACC tracker and extends it with a causal one-dimensional output and explicit quality telemetry. The user selects two or three axes before adding an output; X + Z is recommended, while enabling Y changes the learned three-dimensional direction. A configurable EMA smooths those axes, PCA learns the dominant motion direction over a 12-second quiet calibration, and a 10-second causal baseline suppresses gravity/slow drift. `acc_breathing_magnitude` publishes the signed projection in g. `breathing_volume` is the compatibility ID for its robustly normalized 0–1 waveform and must be interpreted as relative chest-motion effort—not lung volume. Sensitivity maps to the minimum normalized waveform velocity per second required for `breathing_phase`: `+1` inhale, `−1` exhale, and `0` pause/not ready. The classifier derives elapsed time from each notification's ACC sample count at the requested 200 Hz, so equivalent slopes retain their class across different BLE batch sizes. Robust adaptive quantiles track recent scale changes; saving shared settings restarts calibration.

`breathing_signal_ready` requires successful calibration, fresh input, and a minimum all-axis motion score. `breathing_signal_confidence` multiplies calibrated-range, motion, history-coverage, and respiratory-band periodicity factors. It is an app-specific quality index, not a probability or physiological measurement. Not-ready input forces confidence and phase to zero, while the continuous waveform remains available so raw evidence is not silently discarded.
