...
| ACC breathing magnitude estimate | `acc_breathing_magnitude` (continuous projection in g, optionally normalized) |
| Three-state ACC breath-phase classification | `breathing_phase` (+1 inhale, −1 exhale, 0 pause/not ready) |
| Legacy breathing telemetry retained for saved-config migration | `breathing_volume`, `breathing_axis_range`, `breathing_calibration` |
...
The breathing processor follows the original Polar app's ACC tracker while exposing a deliberately smaller public surface. The user selects two or three axes before adding an output; X + Z is recommended, while enabling Y includes the rotational axis. A configurable time window smooths those axes, PCA learns the dominant motion direction over a fixed 12-second quiet calibration, and the continuous `acc_breathing_magnitude` stream publishes that signed projection in g (or an optional 0–1 output transform). Sensitivity maps to the minimum normalized change required for the public three-state `breathing_phase` stream: `+1` inhale, `−1` exhale, and `0` pause/not ready. Incomplete calibration and stale input deliberately remain `0`, so the classifier does not claim a fourth validated signal-quality state. Robust adaptive bounds continue to follow slow posture or strap drift internally; saving shared settings restarts calibration.

Chest-worn accelerometers can estimate respiratory motion and rate, but body movement degrades the signal and methods require validation against a respiratory reference ([Drummond et al., 2021](https://consensus.app/papers/details/9389301a991e52ee9210f51939d318d7/?utm_source=unknown); [Yuasa et al., 2019](https://consensus.app/papers/details/887e8140a6cf5376ab302f44f8b80fb6/?utm_source=unknown); [Schipper et al., 2021](https://consensus.app/papers/details/01e1c8095a4d50c2ab0c5281604ac729/?utm_source=unknown)). Signal-quality rejection can improve respiratory estimates when movement makes windows unreliable ([Romano et al., 2023](https://consensus.app/papers/details/ac9a297ef8f6578699fdc7e56e3c9f16/?utm_source=unknown)). Polar Stream does not expose calibration readiness as a validated quality claim in the simplified ACC library. Both public breathing outputs remain experimental chest-motion estimates—not lung volume, airflow, or clinically validated phase classification.
