...
| ACC breathing waveform | `acc_breathing_magnitude` (signed projection in g), `breathing_volume` (compatibility ID for robust 0–1 normalization) |
| Three-state ACC breath-phase classification | `breathing_phase` (+1 inhale, −1 exhale, 0 pause/not ready) |
| ACC breathing signal state | `breathing_signal_ready`, `breathing_signal_confidence`, `breathing_axis_range`, `breathing_calibration` |
...
The breathing processor follows the original Polar app's ACC tracker and extends it with a causal one-dimensional output and explicit quality telemetry. The default `timed-pca-v1` path applies source-time-aware smoothing to two or three selected axes, learns the dominant motion direction over a 12-second quiet calibration, and projects subsequent samples relative to that fixed calibration center and robust bounds; X + Z is recommended, while enabling Y changes the learned three-dimensional direction. `acc_breathing_magnitude` publishes the signed projection in g. `breathing_volume` is the compatibility ID for its robustly normalized 0–1 waveform and must be interpreted as relative chest-motion effort—not lung volume. The default `hysteresis-v1` phase classifier uses the source-time derivative of the fixed-calibration projection with separate enter/hold thresholds, confirmation, and dwell; `+1` means inferred inhale, `−1` inferred exhale, and `0` pause or not ready. Adaptive output bounds are off by default and, when enabled, do not change the fixed coordinate used for phase. Restored settings without version fields retain the historical `legacy-v0` processor rather than being silently reinterpreted.

`breathing_signal_ready` requires successful calibration, non-lost input, and a minimum all-axis motion score. Under `timed-pca-v1`, `breathing_signal_confidence` multiplies calibrated-range quality, motion quality, and PCA dominance, and is forced to zero when not ready; restored `legacy-v0` settings retain their historical history-coverage and periodicity term. Both confidence formulas are app-specific quality indices, not probabilities or physiological measurements. Not-ready input also forces phase to zero, while the calibrated continuous waveform remains available so raw evidence is not silently discarded.

Chest-worn accelerometers can estimate respiratory motion and rate, but body movement degrades the signal and methods require validation against a respiratory reference ([Drummond et al., 2021](https://pubmed.ncbi.nlm.nih.gov/33018571/); [Liu et al., 2011](https://pubmed.ncbi.nlm.nih.gov/22035321/); [Schipper et al., 2021](https://pubmed.ncbi.nlm.nih.gov/33739305/)). Signal-quality rejection can improve respiratory estimates when movement makes windows unreliable. Polar Stream's exact H10 placement, filters, normalization, readiness threshold, and confidence formula remain experimental and must be evaluated against synchronized GDX-RB Force (N), RIP, airflow, or another justified reference.
